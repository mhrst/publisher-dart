...
Public app-specific values are command options. Credential files can be
supplied with environment variables. The scripts do not read credentials from a
fixed sibling secrets directory.

Android requires:

- `--package-name`
- `--oauth-token` or `GOOGLE_OAUTH_TOKEN`: path to the Google OAuth token JSON
  generated by `oauth-dart`
...
```sh
export GOOGLE_OAUTH_TOKEN="$HOME/.config/example/google-oauth-token.json"
